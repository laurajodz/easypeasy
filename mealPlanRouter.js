...
const {mealPlan} = required('./models/mealPlan');


router.get('/meal-plan', jsonParser, (req, res) => {
  mealPlan
    .find()
    .then(mealPlan => {
      res.json(mealPlan)
...
router.post('/meal-plan', jsonParser, (req, res) => {
  ...
  mealPlan
    ...
    .then(res.status(201).json(mealPlan))
    .catch(err => {
      console.error(err);
      res.status(500).json({message: 'Internal server error'});
    });
...
router.put('/meal-plan/:id', (req, res) => {
